...
public class Physics extends AppCompatActivity {
    ...
    private void SendRequest() {
        ...
        StringRequest request=new StringRequest(StringRequest.Method.POST,Webserviceurl, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    Common.progressDialogDismiss(Physics.this);
                    ...
                    if(total==0)
                    {
                        Toast.makeText(Physics.this,"No Question",Toast.LENGTH_LONG).show();
                    }
                    else
                    {
                        JeeTest.txtquestion.setText(""+total);
                        ...
                        for(int j=0;j<testQuestionArrayList.size();j++) {
                            if(JeeTest.dataflags==false) {
                                if (mydb.inserttest(testQuestionArrayList.get(j).getTst_id(), testQuestionArrayList.get(j).getQ_id(), testQuestionArrayList.get(j).getQuestion(),
                                        testQuestionArrayList.get(j).getA(), testQuestionArrayList.get(j).getB(), testQuestionArrayList.get(j).getC(), testQuestionArrayList.get(j).getD(), "Not Set", 0,testQuestionArrayList.get(j).getSub())) {
                        ...
                        for (int k = 0; k <testQuestionArrayList.size(); k++) {
                            ...
                            Physics.tabLayout.getTabAt(k).setCustomView(tv).setText("" + (k+1));
                        }
            ...
            public void onErrorResponse(VolleyError error) {
                Toast.makeText(Physics.this, ""+error, Toast.LENGTH_LONG).show();
        ...
        Volley.newRequestQueue(Physics.this).add(request);
    }
}
